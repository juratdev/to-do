      ...
      listGroupTodo.innerHTML += `
        ...
        el.isCompleted
          ...
          : "icon-checkbox text-gray-100"
              ...
              <form>
                <input type="text" value="${el.title}" oninput="keyInt(${index}, this.value)" class="w-full bg-inherit outline-none text-3.5xl font-normal
                ${
  ...
  function controlBtns(statusName) {
    const allTabs = document.querySelectorAll('.plan__case__btn');
    allTabs.forEach(tab => tab.classList.remove('bg-orange'));
    
    if (statusName === "all") {
      todos = allTodos;
      const allTab = document.getElementById('all-tab');
      if (allTab) {
        allTab.classList.add('bg-orange');
      }
    } else {
      todos = allTodos.filter((el) => el.status === statusName);
    }
    updateDOM();
    const clickedTab = document.querySelector(`#${statusName}-tab`);
    if (clickedTab) {
      clickedTab.classList.add('bg-orange');
    }
  }
